...
//criando rota que mostre todas as instituições salvas no banco
app.get("/instituicoes", async (req,res)=>{
    try {
        const [resultado] = await conexao.query("SELECT cod_instituicao, nome_instituicao FROM instituicoes");
        res.json(resultado);
    } catch (error) {
...
app.post("/contate_nos", async (req, res) => {
    ...
    try {
        const { nome, email, codigo_instituicao, assunto, mensagem } = req.body;
        ...
        //tratando erros dos usuario sacana
        ...
        } else if (!codigo_instituicao || codigo_instituicao.trim() === ""){
            return res.status(400).json({
                "resposta":"Escolha uma das instituições."
            })
        ...
        const sql = `INSERT INTO contate_nos (nome,email,codigo_instituicao,assunto,mensagem) VALUES (?,?,?,?,?)`;
        const [resultado] = await conexao.query(sql,[nomeTrim, emailTrim,codigo_instituicao, assuntoTrim, mensagemTrim]);
